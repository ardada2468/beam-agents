...
## Intent signing — `beam_agents.intent_signing`

The authenticity half of the intent/result contract: the pipeline signs at the
outbox, and the effector verifies before executing. Pure stdlib (`hmac`,
`hashlib`) — no Beam import, no provider SDK. See
[Effector security](security.md).

| Name | Contract |
| --- | --- |
| `sign_intent` | Return a signed **copy** of a `ToolIntent`. Copying is what keeps keyed state byte-identical. |
| `verify_intent` | Verify an intent against a keyring, returning a `VerificationResult` rather than raising. |
| `VerificationResult` | The four outcomes: ok, `unsigned_intent`, `bad_signature`, `unknown_signing_key`. |
| `signing_input` | The exact bytes signed: the intent's deterministic serialization with the three signature fields cleared. |
| `IntentSigner` | What `WriteIntents` needs to sign: a key id and its secret. |
| `Keyring` | Key id → secret, as the effector resolves it for verification. |
| `load_keyring` | Build a `Keyring` from a secret reference. |
| `SCHEME_HMAC_SHA256` | The one signature scheme this version defines. |
| `resolve_secret_reference` | Resolve an `env:`/`file:` reference to its secret value. |
| `validate_secret_reference` | Validate a secret reference's grammar without reading the secret. |
...
| `TransportSecurity` | Broker credentials by reference (`env:`/`file:`), resolved worker-side at client construction so no secret is pickled into a pipeline graph. |
| `VerificationMode` | The signature-verification dial: `off`, `permissive`, `require`. |
| `VERIFICATION_MODES` | The closed set of accepted `VerificationMode` values. |
| `redact_uri` | Strip credentials from a URI before it reaches an error message, a `repr`, or a log line. |
| `transport_security_from_args` | Build a `TransportSecurity` from parsed CLI arguments, or `None` when nothing was configured. |
