...
COPY src /src/src
# `langgraph`/`langchain-core` mirror the `langgraph` extra and `google-adk`
# mirrors the `adk` extra: the adapter conformance matrix's Flink leg runs both
# framework adapters' cells inside this harness, so each framework must be
