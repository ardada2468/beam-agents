...
**Resolution:** duration's home landed on `main` mid-implementation as the `runtime-metrics` capability (`add-runtime-metrics`, merged into this branch): `beam_agents.runtime/activation_ms`, the release-gating `overhead_ms`, `llm_ms`, and per-outcome counters, all recorded at commit from a per-activation tally with injected monotonic clocks — no clock in any trace byte, so replayed bundles still emit byte-identical records and the no-wall-clock requirements stand. (This change briefly carried its own smaller metrics module before the merge; it was dropped in favor of the richer capability.) Span *durations* remain deliberately zero-width; anyone reaching for latency has a real home for it instead of a misleading span width.
...
- ~~Should `observability/metrics.py` introduce the injected monotonic seam D7 declines to add?~~ **Resolved by `add-runtime-metrics`** (merged from `main` mid-implementation) — see D7's resolution note. The seams live on the DoFn and contexts (`monotonic_ns=`), latency reports as Beam metrics, and trace bytes stay deterministic. The narrower question that remains open is whether `beam_agents.duration_ms` should ever be backfilled onto the events themselves; doing so would break byte-identical replay and content-level dedup, so the current answer is no.
