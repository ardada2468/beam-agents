...
`effector/` imports only `beam_agents.tools`, `beam_agents.hitl`, and `beam_agents._protos` — all three are already Beam-free and side-effect-free on import.

**The boundary is over the closure, not the import statement.** Implementation surfaced a fact the first draft of this design glossed over: `import beam_agents.effector` necessarily executes the parent `beam_agents/__init__.py`, which re-exports `RunAgent` and therefore imports Beam. Nothing short of moving the effector to a separate top-level package or making the parent package lazy would change that, and both cost more than they buy for a v0.x reference service whose distribution depends on Beam anyway. So the guarantee is stated over the effector's own dependencies, and verified two ways: statically, by walking every effector module's AST and rejecting any `apache_beam` or `beam_agents.core` import (which catches lazy and guarded imports too), and dynamically, by importing every submodule in a subprocess with those roots hard-blocked and the parent package stubbed — which is exactly the view a standalone deployment of these modules has. Concrete client libraries (`aiokafka`, `google.cloud.pubsub_v1`, `redis.asyncio`, `google.cloud.bigtable`) are imported **inside** the adapter constructors, the same lazy-import pattern (and the same `PLC0415` per-file ignore) already used by `core/transform.py` and `actions/write_intents.py`.
