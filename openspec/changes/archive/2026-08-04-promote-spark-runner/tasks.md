...
- [x] 6.1 Before archiving, confirm `add-adapter-conformance-matrix` has archived and `openspec/specs/adapter-conformance-matrix/spec.md` exists with the base requirement headers this change's delta renames/modifies (design D8); if its text drifted, reconcile this change's delta verbatim before archiving
  ...
  - *Done at archive time (2026-08-03):* `add-adapter-conformance-matrix` archived as `archive/2026-08-04-add-adapter-conformance-matrix` and `openspec/specs/adapter-conformance-matrix/spec.md` now exists. Reconciliation performed header-by-header: the RENAMED `FROM` header (`The matrix runs on DirectRunner and Flink`) is present verbatim in the main spec; the renamed requirement's MODIFIED block carries all three landed scenarios (`Offline leg needs no docker`, `Flink leg survives a TaskManager restart mid-suspension`, `A leg-inexpressible scenario is an explicit skip`) plus the new Spark scenario; `The matrix cannot silently lose cells` carries the landed `Wiring regression is caught by the meta-test` scenario plus the new Spark-cell scenario. No drift found; the delta needed no edits.
...
- [ ] 7.4 `make coverage-ratchet`
  ...
  - *Addendum (2026-08-03, at archival):* the baseline has since been re-measured on the integrated tree (commit f84f019) and `add-1-0-0-release` task 5.7 ran `make coverage-ratchet` green at 91.64%, at baseline — the reconciliation this note asked for happened. Archiving now is correct despite this box and 2.2/3.5: the 1.0 release gate (`add-1-0-0-release` task 2.5) records this change's decision as **deferred, not promoted** with exactly these items open — the spike and local-stack run are docker-blocked and the promotion clock starts with the first scheduled weekly run, none of which the delta specs depend on.
