...
The evidence-driven process by which the Spark portable runner moves between best-effort and supported status: a spark conformance leg with per-scenario declarations, a weekly (never per-PR) CI workflow, mechanical promotion-window reporting, a four-consecutive-green-weeks promotion gate, and a two-red-weeks demotion path.
