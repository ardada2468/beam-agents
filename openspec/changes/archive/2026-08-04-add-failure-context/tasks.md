...
- [x] 4.1 Archive after `add-trace-events`: this change's delta modifies the `trace-events` requirement that `add-trace-events` introduces, so its spec sync must land on top of that capability's main spec.
  - *Done (2026-08-03):* `add-trace-events` archived first (as `archive/2026-08-04-add-trace-events`, landing `openspec/specs/trace-events/spec.md`); this change archived immediately after it, so the ordering constraint held.
