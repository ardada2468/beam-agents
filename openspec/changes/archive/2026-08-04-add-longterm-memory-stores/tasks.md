...
- [x] 10.4 Coverage ratchet (`make coverage-ratchet`) does not regress; mutation gate on touched `core/` files passes. — discharged by close-core-mutation-gaps, which ran both on the combined tree: `make mutation` passes (2475 core mutants — killed 1975, no tests 477, survived 23, all declared equivalents), and `make coverage-ratchet` is at its raised baseline (0.9180, up from 0.9164). This change's own survivor was `_activate` dropping the `longterm_store` it hands the activation, now killed by `test_dofn_activation.py`'s store-wiring tests, together with `loop.py`'s `LongtermFlushFailed` construction and the `_flush_longterm` guard. Original note follows. — not executed in that environment (the ratchet needs a full-lane `coverage.xml` and the mutation gate forks a pytest run per mutant, both outside this offline session's budget); left for CI's `quality` job.
