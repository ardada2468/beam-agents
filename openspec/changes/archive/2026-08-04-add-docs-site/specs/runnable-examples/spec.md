...
Three runnable example pipelines — hello-world, fraud triage, IoT reaction — that demonstrate the runtime's documented workloads as real Beam pipelines driven by FakeLLM, run offline in the unit tier with no credentials or docker, and are held to the doc-example-verified-by-test contract so every claim a docs page makes about them is asserted by a test.
