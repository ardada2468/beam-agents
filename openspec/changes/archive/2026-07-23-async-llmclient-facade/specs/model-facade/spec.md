...
The facade SHALL stage exactly one `TraceEvent` of type `LLM_CALL` per `complete` invocation into the staging context, carrying the activation `entity_key`, `seq`, and `step_index`, and OTel-GenAI-shaped string attributes including at least: `gen_ai.request.model`, the token usage (`gen_ai.usage.input_tokens`, `gen_ai.usage.output_tokens`), the cache outcome (hit/miss), the number of attempts, and the endpoint circuit-breaker state. `start_ms`/`end_ms` SHALL be taken from the injected clock. Trace events SHALL be staged like every other activation effect and applied only on activation success; a `complete` that ultimately raises SHALL still stage a trace event describing the failure.

#### Scenario: A successful call emits one LLM_CALL trace

- **WHEN** `complete` returns successfully
- **THEN** exactly one `LLM_CALL` `TraceEvent` is staged with `gen_ai.request.model` set, usage and attempt attributes populated, and the cache outcome recorded

#### Scenario: A cache hit is recorded in the trace attributes

- **WHEN** `complete` resolves from a cache hit
- **THEN** the staged `LLM_CALL` trace records the cache-hit outcome and zero attempts

#### Scenario: A failed call still emits a trace

- **WHEN** `complete` exhausts retries and raises
- **THEN** an `LLM_CALL` `TraceEvent` describing the failure (error attribute and attempt count) is staged before the exception propagates
