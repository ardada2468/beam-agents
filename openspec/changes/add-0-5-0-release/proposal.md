...
**New code:** none. The only additions are the `0.5.0` changelog section and this change's planning artifacts.

**Modified code:** `pyproject.toml` (version line only) and the changelog file established by `add-0-1-0-release`. No `src/`, `tests/`, or proto changes.
