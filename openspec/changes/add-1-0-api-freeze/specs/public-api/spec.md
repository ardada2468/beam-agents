...
`beam_agents/__init__.py` SHALL re-export exactly eleven names: `AgentConfig`, `Deny`, `Drop`, `Escalate`, `FallbackContext`, `HitlPolicy`, `LangGraphAgent`, `RunAgent`, `RunAgentOutputs`, `StreamAgent`, and `tool`, and `__all__` SHALL list exactly these. `tool` and `StreamAgent` MUST resolve eagerly; `LangGraphAgent` MUST continue to resolve lazily via module `__getattr__`, raising an `ImportError` that names the extra to install when the LangGraph adapter's dependencies are absent. Importing the package MUST remain free of side effects.

The code-style section of `openspec/project.md` SHALL enumerate the same eleven names, so the constitution and the code state one surface.

#### Scenario: The root surface matches the frozen list

- **WHEN** `beam_agents` is imported and its `__all__` compared against the eleven frozen names
...
Every top-level name in `src/beam_agents` SHALL be either part of the audited public contract — the root tier or a module-tier name listed in its module's `__all__` — or prefixed with a leading underscore. Modules that exist only as internal machinery SHALL expose no public contract names. The audit SHALL resolve every name that is public today but internal in intent — including the LangGraph transport helpers, the provider `decode` functions, and effector wiring internals — by underscore-renaming rather than by documenting them into the contract.
...
Every public module, class, function, and method in `src/beam_agents` SHALL have a docstring. Enforcement SHALL be ruff's pydocstyle completeness rules (the `D1` selector) added to the existing `[tool.ruff.lint]` `select`, running inside the existing `make lint` target with no new tool or CI step. `D105` (magic methods) and `D107` (`__init__`) SHALL be ignored — dunder semantics are defined by the language protocol, and constructor contracts belong in the class docstring — and `tests/*` SHALL be exempted from `D1` via the per-file-ignores table. The gate MUST be a per-name check, not a percentage threshold: a single new undocumented public name fails the build regardless of overall coverage.
...
The repository SHALL ship an API reference page under `docs/` enumerating the frozen public surface: every root re-export and every module-tier public name grouped by its module, each with a one-line statement of its contract. A unit test SHALL assert that every name recorded in the surface snapshot appears on the page, so the reference cannot silently fall behind the frozen surface.
