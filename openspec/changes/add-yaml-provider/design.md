...
Input: the wrapped transform reads `key_field` (default `"key"`) and `payload_field` (default `"payload"`) off each row, encodes a `str` key as UTF-8 bytes (bytes pass through), and builds `AgentEnvelope(entity_key=..., event_time_ms=..., external_event=<payload bytes>)`, keying with the same `WithKeys(...).with_output_types(tuple[bytes, AgentEnvelope])` idiom the core transform documents — so `RunAgent`'s KV validation sees exactly the shape it requires. `event_time_ms` comes from `event_time_field` when configured, else the element timestamp — with Beam's unstamped sentinel (`MIN_TIMESTAMP`, ~292M years pre-epoch, what a bounded `Create` or an unstamped source carries) mapped to `0`, since carrying it into the envelope would poison `expires_at_ms`, the TTL mark, and the trace rows' RFC 3339 `event_time`. The mapping reads no clock, so it stays replay-deterministic. A row missing the configured fields is a data error, not a config error: it routes to the `errors` output as a malformed-input record rather than crashing the bundle, consistent with the project's route-element-failures-to-errors rule.

Output: YAML consumers need schema'd rows, and two of the four streams already have row mappers shipped for the BigQuery sinks — `trace_event_to_row` and `activation_error_to_row`, imported today by [`core/transform.py`](../../../src/beam_agents/core/transform.py:45). The YAML transform reuses those for `traces`/`errors`, adds the equivalent scalar-field mapping for `intents` (`intent_id`, `entity_key`, `seq`, `step_index`, `tool_name`, `args_json`, `created_at_ms`, `expires_at_ms`, `attempt`, `kind`), and emits the main output as `Row(output: bytes)` — `RunAgent`'s main stream is an unkeyed `PCollection[bytes]` (the DoFn yields `result.outputs` verbatim), so there is no entity key on it to carry, and the runtime imposes no schema on the bytes either, so none is invented; a downstream YAML `MapToFields` can decode them. An agent that needs its key downstream emits it inside its own output bytes. Sink URIs (`intents_to` etc.) remain the recommended drain path from YAML, in which case those tagged row streams simply go unconsumed; the row conversion exists so consuming them in-DAG is *possible*, not mandatory.
...
The YAML-facing transform exposes the wrapped `RunAgentOutputs` under Beam YAML's multi-output convention with the same four names the Python surface uses, `output` being the main stream. A YAML consumer addresses the non-main streams by qualified name (Beam YAML's `TransformName.output_name` input addressing). Keeping the names identical to the [`RunAgentOutputs`](../../../src/beam_agents/core/transform.py:438) attributes means docs, traces, and both surfaces agree on vocabulary. **Resolved:** the conditional `dead_letter` stream is *not* exposed as a fifth named output. A sometimes-present output would make a document's validity depend on whether `intents_to` happens to resolve to the outbox writer, which is exactly the kind of implicit coupling the fixed key set exists to prevent. Instead the YAML layer keeps the output set constant at four, and malformed input rows (D5) join the activation dead letters on `errors` so one stream is the whole failure story.

**Resolved against Beam 2.72.0's source:** `expand_leaf_transform` names outputs from a `dict[str, PCollection]` return; a tuple/list degrades to `out0`/`out1`/..., and any other return type (including a dataclass like `RunAgentOutputs`) is rejected outright. `expand` therefore returns a dict keyed by the four names. `Scope.get_pcollection` addresses one as `TransformName.output_name`, and a transform with more than one output cannot be addressed bare — so a YAML consumer always names the stream it wants, `.output` included.
...
**Verified against Beam 2.72.0:** `python` is a registered provider type. With a `packages:` list it builds an `ExternalPythonProvider`, which installs those distributions into a managed venv and expands there; with no `config:` at all it builds an `InlineProvider` that resolves the fully-qualified constructor *in-process* via `PythonCallableWithSource.load_from_source` and calls `factory(**config)`. Note the two different spellings the docs must keep straight: the provider's `transforms:` values are all-dots fully-qualified names, while our own config values are `module:object` references. This is why the change depends on C25: `packages:` must name something installable.
...
Resolved during implementation by reading the installed Apache Beam 2.72.0 source; each answer is recorded in the module that depends on it and pinned by a drift-guard test (`tests/yaml/test_yaml_e2e.py::test_beam_yaml_contract_still_matches_this_driver`).

- ~~**Exact Python-provider declaration for the pinned Beam version.**~~ **Resolved (D7):** `type: python`. With `config: {packages: [...]}` it is an `ExternalPythonProvider` expanding in a managed venv; with no `config:` it is an `InlineProvider` resolving the constructor in-process via `PythonCallableWithSource.load_from_source`. No expansion service is ours to ship either way.
- ~~**Multi-output declaration mechanism.**~~ **Resolved (D6):** a `dict[str, PCollection]` return from `expand`; consumers address `TransformName.output_name`, and a multi-output transform cannot be addressed bare.
- ~~**Conditional `dead_letter` output.**~~ **Resolved (D6):** not exposed. The output set stays constant at four; malformed input rows merge onto `errors`.
- ~~**Test-environment dependency.**~~ **Resolved:** `apache_beam.yaml` does *not* import under the plain `apache-beam[gcp]` pin. It needs Beam's `yaml` extra — `jinja2` (imported by `yaml_transform`) and `pandas` (via `yaml_provider` → `apache_beam.dataframe.io`), plus `virtualenv-clone`/`js2py` for the package-installing provider path. Of those the locked test environment carries none. See tasks.md Revision 1 for what the e2e suite does instead.
- **Provider discovery.** Still open, still deferred (D7): Beam's `parse_providers` reads a document's `providers:` list and supports `- include: <path or URL>`, which the packaged listing file targets; no entry-point group is consulted. Automatic registration would need a Beam-side change.
- **Secret handling.** Still open. Docs steer to document templating (`docs/yaml.md`); an interpolating `env:VAR` arm interacts with `provider_factory` pickling and belongs to a follow-up.
- **Shorthand provider names.** Still open (D3). The long-form reference convention is uniform across all five reference positions; an alias table can be layered on without breaking it.
