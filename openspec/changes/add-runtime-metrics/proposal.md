...
- **No public API change.** `observability` is internal; nothing new is re-exported from `beam_agents/__init__.py`, and no `AgentConfig` knob is added. The recorder is injectable through a private DoFn constructor parameter for tests only.
...
- **`tool_calls` starts at zero in the shipped pipeline.** Inline tools run through `AgentContext.run_tool`, and the DoFn currently drives `ActivationContext`, which has no inline-tool surface. The counter is defined and wired at the execution site so it begins reporting the moment the adapter path lands, rather than being retrofitted then.
