...
`pyproject.toml` SHALL declare `requires-python = ">=3.10,<3.13"` and MUST include `apache-beam[gcp]>=2.60`, `httpx[http2]`, `pydantic>=2`, and `protobuf` in `[project.dependencies]`. Attempting to install on an unsupported interpreter MUST fail with a clear resolver error.
...
The repository SHALL define four workflows under `.github/workflows/`: `ci.yml`, `integration.yml`, `quality.yml`, and `nightly.yml`. `ci.yml` MUST run a matrix of Python `3.10`, `3.11`, `3.12` on `ubuntu-latest` and `macos-latest`, executing `make lint type test-unit`. `integration.yml` MUST run `make compose-up test-integration test-semantics` on `ubuntu-latest`. `quality.yml` MUST run `make mutation` plus a coverage-ratchet check against `main`. `nightly.yml` MUST run on a `0 7 * * *` schedule and on `workflow_dispatch`, authenticating to GCP via Workload Identity Federation and running `-m dataflow` tests. `ci`, `integration`, and `quality` MUST be marked required for merge into `main`.
