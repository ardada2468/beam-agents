...
- [ ] 1.1 Record the host in `verification-report.md`: OS, CPU count, total RAM, `docker --version`,
  `docker compose version`, `uv --version`, `python --version`, and `git rev-parse HEAD`.
- [ ] 1.2 Confirm Docker has **at least 8 GB** of memory allocated (the full stack runs Flink
  ...
  failures.
- [ ] 1.3 `uv sync --locked --all-groups` — this is the only sync that installs `precommit`
  ...
  these were absent.
- [ ] 1.4 Re-confirm the offline baseline still passes on your machine before adding any variables:
...
- [ ] 1.5 `uv run pre-commit run --all-files` — **never executed in the implementing environment**
...
- [ ] 2.1 `make compose-up-core` — starts only Redpanda, Redis, and the Pub/Sub and Bigtable
  emulators (no Flink). Confirm all four report healthy: `docker compose -f docker/compose.yaml ps`.
- [ ] 2.2 `make test-integration` — the `integration and not semantics` selection, **119
...
- [ ] 2.3 Triage any failure per D2. Common `infra` signatures here: connection refused (service not
...
- [ ] 2.4 The Firestore memory-store leg needs the `firestore-emulator` service (present in
...
- [ ] 2.5 Record the phase verdict and evidence in the report.

## 3. Phase 2 — Flink: conformance matrix and the effectively-once gate

- [ ] 3.1 `make compose-up` — the full stack including Flink and the SDK harness. First run builds
...
- [ ] 3.2 `make test-conformance-flink` — **28 cells (4 adapters × 7 scenarios) that have never
...
- [ ] 3.3 Note the one declared adapter skip: ADK's `bundle_retry_cache` cell is a documented
...
- [ ] 3.4 `make test-semantics` — **`tests/semantics/test_effectively_once_e2e.py`, the release gate
...
- [ ] 3.5 If 3.4 is slow or flaky on your machine, you may first smoke it with
...
- [ ] 3.6 On failure, read the seed line the harness logs (`run seed=<n> … rerun with
  BEAM_AGENTS_E2E_SEED=<n>`) and reproduce deterministically before triaging. The kill schedule is
  derived from that seed.
- [ ] 3.7 On any red docker phase, capture diagnostics **before** teardown: `make compose-logs
...
- [ ] 3.8 Record verdicts and evidence for 3.2 and 3.4 separately — they are distinct gates.

## 4. Phase 3 — Spark leg (first execution ever)

- [ ] 4.1 `make compose-up-spark` — brings up the Spark job server overlay
  (`docker/compose.spark.yaml`) alongside the base stack.
- [ ] 4.2 `make test-conformance-spark` — **28 cells, never run against a job server.** Per D3 this
...
- [ ] 4.3 Triage each failure into: (a) infra — job server unreachable, artifact staging failure;
...
- [ ] 4.4 For any (b): **do not edit the declaration inline.** File a change converting that
  scenario's `Run()` to `Skip(reason=...)` with the evidence — that is `promote-spark-runner`'s own
  documented mechanism.
- [ ] 4.5 Record the verdict. Note in the report that a successful local run is evidence the leg
...
- [ ] 4.6 `make compose-down-spark` when finished.

## 5. Phase 4 — quality gates that need the full lane

- [ ] 5.1 `make mutation` — **never run on this tree**; five changes deferred it. Generates ~900
...
- [ ] 5.2 If the gate fails, list the surviving mutants in the report. Raising a ceiling or adding an
  ...
  renumbering.
- [ ] 5.3 Re-measure coverage over the **full** lane (offline + integration) rather than the offline
...
- [ ] 5.4 Record both verdicts.

## 6. Phase 5 — benchmarks and baseline seeding

- [ ] 6.1 Decide whether this host qualifies to seed medians (D3, and the requirement that the host
...
- [ ] 6.2 `make bench` — runs all five dimensions into `bench-results/`: no-op throughput, FakeLLM
  overhead tiers (50/500/2000 ms), suspension round-trip, state-commit cost vs `MemoryBlob` size,
  and the `RunInference` comparison.
- [ ] 6.3 `make bench-gate` — enforces the absolute budget (**p50 < 15 ms, p99 < 60 ms** over pooled
  ...
  ratchet against yet.**
- [ ] 6.4 If the host qualifies: seed `benchmark-baseline.toml`'s `[medians_ms]` from the measured
  values, re-run `make bench-gate` to confirm it is now at baseline, and record the seeding host in
  the report.
- [ ] 6.5 Record the budget verdict and the seeding decision.
- [ ] 6.6 `make compose-down` — tear the local stack down.

## 7. Phase 6 — Dataflow (when GCP is provisioned)

- [ ] 7.1 Configure and record: `GCP_PROJECT_ID`, `GCP_REGION`, `GCP_DATAFLOW_TEMP_BUCKET`,
...
- [ ] 8.1 `make test-smoke` with `ANTHROPIC_API_KEY` and/or `OPENAI_API_KEY` set — 3 `smoke` tests
  against live providers. Optional; records real-provider behavior. **(blocked without keys)**
- [ ] 8.2 `tests/smoke/test_vllm_sidecar.py` on a GPU host — the only path that exercises
  `VllmSidecarProvider`'s real engine rather than its fake-engine seam. **(blocked without a GPU)**
- [ ] 8.3 Record both as `blocked` with the missing prerequisite if unavailable — do not omit them.

## 9. Report, discharge, and findings

- [ ] 9.1 Write `verification-report.md`: commit verified, host details, compose image identifiers,
...
- [ ] 9.2 Discharge the blocked task items across the 26 implemented change folders — **105 unchecked
...
- [ ] 9.3 Commit the task-item discharge separately from the report and baselines, so a reviewer can
  read the wide `tasks.md` diff independently of the substantive artifacts.
- [ ] 9.4 File one OpenSpec change per defect found. Do **not** fix defects in this change.
- [ ] 9.5 If any release-gate condition is now satisfiable, note it in the report — but do **not**
...
- [ ] 10.1 `make lint` and `make type` clean at the end of the run (nothing this change does should
  move them).
- [ ] 10.2 `make test-unit` and `make test-semantics-offline` still green — confirming the run did
  not perturb the offline tier.
- [ ] 10.3 `make coverage-ratchet` at or above the re-measured baseline.
- [ ] 10.4 `uv run pre-commit run --all-files` clean.
- [ ] 10.5 `npx --yes @fission-ai/openspec@1.7.0 validate verify-live-infrastructure --strict` passes.
- [ ] 10.6 `verification-report.md` committed, with every phase carrying a verdict.
